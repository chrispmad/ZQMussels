---
title: "Golden Mussel Waterbody Sampling Shortlist"
author: "John Phelan"
date: "`r Sys.Date()`"
output:
  rmdformats::robobook:
...
```{r read_in_data}
# Read RDS.
dat = read_rds(paste0("C:/Users/JPHELAN/Documents/mussel_model/data_for_making_shortlist_golden_mussel.rds"))
# if(interactive()){
...
queried_wbs<- waterb_og_values_and_bins |> 
  filter(str_detect(GNIS_NA, "Kentucky|Alleyne")) |> 
  st_drop_geometry()

write.csv(queried_wbs, "./output/qeuried_wbs_golden_mussel.csv")
...
nr_regions = sf::read_sf(paste0(lan_root,"2 SCIENCE - Invasives/AIS_R_Projects/CMadsen_Wdrive/shared_data_sets/FLNRO_Fishing_Boundaries.shp"))
...
sample_protocol_2022 <- get_wb_list_from_protocol_doc(
  paste0(lan_root,"2 SCIENCE - Invasives/SPECIES/Zebra_Quagga_Mussel/Operations/Lake Monitoring/2022/Sampling protocol/2022 Invasive Mussel Field Protocol_Draft_V1.docx")
)

# 2023 protocol
sample_protocol_2023 <- get_wb_list_from_protocol_doc(
  paste0(lan_root,"2 SCIENCE - Invasives/SPECIES/Zebra_Quagga_Mussel/Operations/Lake Monitoring/2023/Sampling protocol/2023 Invasive Mussel Field Protocol_Draft_V1.docx")
)

# 2024 protocol
sample_protocol_2024 <- get_wb_list_from_protocol_doc(
  paste0(lan_root,"2 SCIENCE - Invasives/SPECIES/Zebra_Quagga_Mussel/Operations/Lake Monitoring/2024/Sampling protocol/2024 Invasive Mussel Field Protocol_Final.docx")
)

# 2025 protocol
sample_protocol_2025 <- get_wb_list_from_protocol_doc(
  paste0(lan_root,"2 SCIENCE - Invasives/SPECIES/Zebra_Quagga_Mussel/Operations/Lake Monitoring/2025/Sampling protocol/2025 Invasive Mussel Field Protocol_V2.docx")
)
...
past_sample_protocols <- sample_protocol_2021 |> 
  ...
  bind_rows(sample_protocol_2025 |> mutate(Lat = as.numeric(Lat), Long = as.numeric(Long)) |> mutate(sampling_year = 2025))
...
sample_protocol <- sample_protocol %>%
  mutate(
    # replace anything that contains Coeur ... Alene ... Creek (case-insensitive)
    Waterbody = if_else(
      str_detect(Waterbody, regex("(?i)Coeur.*Alene.*Creek")),
      "Coeur d’Alene Creek",
      Waterbody
    )
  ) %>%
...
waterb_risk = waterb_risk |> 
  dplyr::mutate(keep_me_always = dplyr::case_when(
    ...
    GNIS_NA %in% c('Alleyne Lake') ~ T,
    GNIS_NA %in% c('Kentucky Lake') ~ T,
...
saveWorkbook(my.wb,
            paste0(my_opts$zqm_operations_data_folder,"Lake Monitoring/",my.year,"/Prioritization model/waterbodies with risk estimates post filtering_",my.year,"_golden_mussel.xlsx"),
            overwrite = T)
```
...
openxlsx::write.xlsx(final_waterbody_list,
                    paste0(my_opts$zqm_operations_data_folder,"Lake Monitoring/",my.year,"/Prioritization model/Final waterbody list based on risk estimates_",my.year,"_golden_mussel.xlsx"),
                    overwrite = T)

# Add a sheet to this doc that has only the variables needed for the table in word.
final_wb_wb = openxlsx::loadWorkbook(file = paste0(my_opts$zqm_operations_data_folder,"Lake Monitoring/",my.year,"/Prioritization model/Final waterbody list based on risk estimates_",my.year,"_golden_mussel.xlsx"))
...
openxlsx::saveWorkbook(final_wb_wb, 
                       file = paste0(my_opts$zqm_operations_data_folder,"Lake Monitoring/",my.year,"/Prioritization model/Final waterbody list based on risk estimates_",my.year,"_golden_mussel.xlsx"),
                       overwrite = T)
```
...
last_year = get_wb_list_from_protocol_doc(paste0(lan_root,"2 SCIENCE - Invasives/SPECIES/Zebra_Quagga_Mussel/Operations/Lake Monitoring/2025/Sampling protocol/2025 Invasive Mussel Field Protocol_V2.docx")) |> 
...
writeDataTable(wb = my.wb,
                   ...
                   withFilter = T)
saveWorkbook(my.wb,
            paste0(my_opts$zqm_operations_data_folder,"Lake Monitoring/",my.year,"/Prioritization model/Sampling_list_comparison_",my.year,"_with_",my.year-1,"_golden_mussl.xlsx"),
              overwrite = T)
```
