---
title: "Golden Mussel Waterbody Prioritization Models"
author: "John Phelan"
date: "`r Sys.Date()`"
output:
  rmdformats::robobook:
...
onedrive_wd = "C:/Users/JPHELAN/OneDrive - Government of BC/data/"
...
This R Markdown document estimates the invasion risk of Zebra-Quagga Mussels in freshwater waterbodies in British Columbia. Please note: only named waterbodies have been included in this analysis! Also, for the final introduction and economic risk maps, only waterbodies with at least one watercraft inspection record are retained.
...
# waterb_with_data_explored = readxl::read_excel("J:/2 SCIENCE - Invasives/SPECIES/Zebra_Quagga_Mussel/Operations/Watercraft Inspection Data/Multiyear data/waterb_with_data_explored.xlsx")

# waterb$number_of_lakes_in_subwatershed = waterb_with_data_explored$number_of_lakes_in_subwatershed
```

```{r ensure_water_vars_are_factors}
waterb = waterb |> 
...
## QUESTION - do we need to bring in dam data here? Haven't we done this in script 01_ZQM_data_preparation?
...
# CDC listed species
cdc_sar_sp = sf::read_sf('C:/Users/JPHELAN/OneDrive - Government of BC/R_projects/one_off_projects/AEB_species_at_risk_visualizer_2024/app/www/species_ecosystems_at_risk_publically_available_no_insects.gpkg') |>
  sf::st_transform(3005)

fed_sara_sp = sf::read_sf(paste0(onedrive_wd,"CNF/DFO_SARA_occ_data_QGIS_simplified.gpkg")) |> 
  sf::st_transform(3005) |> 
  sf::st_make_valid()
...
cdc_sar_sp_by_waterb = cdc_sar_sp |> 
  st_join(waterb, st_intersects)
  
cdc_sar_sp_by_waterb = cdc_sar_sp_by_waterb |> 
  sf::st_drop_geometry() |> 
  dplyr::filter(!is.na(GNIS_NA)) |> 
  dplyr::group_by(GNIS_NA,WATERBO,WATERSH) |> 
  summarise(CDC_listed_SAR = paste0(SCI_NAME, collapse = ', ')) |> 
  dplyr::ungroup() |> 
  dplyr::mutate(distinct_CDC_listed_SAR = stringr::str_detect(CDC_listed_SAR,',') + 1)

waterb = waterb |> 
  dplyr::left_join(cdc_sar_sp_by_waterb)

fed_sara_sp_by_wb = fed_sara_sp |> 
  st_join(waterb, st_intersects)
  
fed_sara_sp_by_wb = fed_sara_sp_by_wb |> 
  sf::st_drop_geometry() |> 
  dplyr::filter(!is.na(GNIS_NA)) |> 
  dplyr::group_by(GNIS_NA,WATERBO,WATERSH) |> 
  summarise(Fed_SARA_listed_SAR = paste0(Scientific_Name, collapse = ', ')) |> 
  dplyr::ungroup() |> 
  dplyr::mutate(distinct_Fed_SARA_listed_SAR = stringr::str_detect(Fed_SARA_listed_SAR,',') + 1)

waterb = waterb |> 
  dplyr::left_join(fed_sara_sp_by_wb)
...
lakes_m = waterb |> 
  sf::st_intersection(watershed_m)
...
# Note: The risk level for waterbodies with pH or (air) temperature (averaged at the subwatershed scale) outside of suitable ranges for Zebra Quagga Mussel establishment was reduced by 1.
```
...
-   Five categories of variables were defined:
    1.  Use (Total Watercraft Inspections, Days fished (2023 survey), and number of marinas)
    2.  High-risk (High-risk Inspections and Transport Canada Operation Restrictions)
    3.  Mussel-Fouled (Mussel-fouled Inspections)
    4.  Subwatershed-scale mean dissolved calcium, corrected for pH and temperature
```{r}
    # 5.  Water Abiotic Variables (pH, temperature, total phosphorous, chlorophyll, dissolved oxygen; interpolated EMS data at provincial scale)
```


### Economic Value Risk

-   Three categories of variables were defined:
    1.  Summed Capacity of Hydroelectric Dams by subwatershed (dam points buffered by 100m)
    2.  Species Richness of Federally SARA-listed Aquatic Species-at-risk by subwatershed
    3.  Species Richness of Provincial Conservation Data Center listed Aquatic Species-at-risk by subwatershed
...
Some of the variables were classified differently. Operational restrictions on power/electric propulsion of vessels were assigned a value of -2, and restrictions on power propulsion were assigned a value of -1. Mussel-fouled inspections were waterbodies with any mussel-fouled inspections were assigned a value of 1 for the variable *M*, while those waterbodies without mussel-fouled inspections received a 0 for *M*. Mean dissolved calcium was binned into four levels prior to inclusion in this analysis: no risk (\<8 mg/L), low risk (8 to \<15 mg/L), moderate risk (15 to \<30 mg/L), and high risk (30+ mg/L) (water samples with dissolved chemistry greater than 300 mg/L were deemed to be erroneous data points and were dropped). Other water chemistry variables were binned according to the following table (small corrections were made to these ranges when they unintentionally left gaps).
...
The following risk models were constructed with *a priori* knowledge of key variable inputs:

$$I_w = (\frac{T_w+A_w+M_w-O_w}{n}) + (\frac{H_w}{n}) + (\frac{MF_w}{n}) + {C_w}$$

Where *Iw* represents the invasion / establishment risk estimate of a waterbody *w*. *T* refers to the total number of watercraft inspections of watercraft headed to waterbody *w*, *A* to angling effort (sum of days fished), *M* to the number of marinas in waterbody *w*, *O* *O* refers to operational restrictions, and *H* to high-risk inspections. *MF* to mussel-fouled inspections, and *C* stands for the subwatershed-level calcium level (calculated using Environment Monitoring System data). *O* represents a risk reduction, and so it is subtracted from the Use category estimate.

Note that angling effort was found to inflate the number of waterbodies categorized as risk level 2 and 3; therefore, this variable's weighting was reduced relative to total inspections and marinas by condensing its value range from 1-3 to 1-2 (a value of 1 remained 1, a value of 2 became 1.5, and a value of 3 became 2).

$$EV_w = (D_w) + (FSARA_w) + (PSAR_w)$$ Where *EV_w* represents the economic value risk estimate of a waterbody. *D* refers to the summed dam capacity at the waterbody scale, *FSARA* refers to the number of distinct species of aquatic, federally SARA-listed species-at-risk, and *PSAR* refers to the number of distinct species of aquatic, provincially listed by the Conservation Data Centre as species-at-risk, all at the waterbody scale.
...
#Define variable groupings. 'all.my.vars' get binned, while variables outside of that will not.
all.my.vars = c("TotalInspections",
                "days_fished_2023_survey",
                "Marinas",
                "HR_mot",
                "HR_nonmot",
                "OperRes",
                "NumberMusselFouled",
                "SummedDamCapacity",
                "distinct_CDC_listed_SAR",
                "distinct_Fed_SARA_listed_SAR"
                )

MusselFouledvar = "NumberMusselFouled"

Usevars = c("TotalInspections",
            "days_fished_2023_survey",
            "Marinas",
            "OperRes")

Highriskvars = c("HR_mot","HR_nonmot")

# WaterSuitabilityVars = c(
#   "water_temp_interp",
#   "water_chlor_interp",
#   "water_dis_ox_interp",
#   "water_tot_phos_interp",
#   "water_pH")

Damvars = c("SummedDamCapacity")
SARvars = c("distinct_CDC_listed_SAR","Fed_SARA_listed_SAR")
...
#in the script.
waterb = waterb |> 
dplyr::rename(HR_nonmot = HR_Non_Motorized_Counter)
...
data_long = waterb %>%
  st_drop_geometry() %>% 
  #Keep only the variables we've chosen for the groups of variables.
  ...
  # Count up the number of CDC listed and Fed SARA listed species
  # dplyr::select(-c(CDC_listed_SAR,Fed_SARA_listed_SAR)) |> 
...
data_bin = data_long %>% 
  ...
  # nest() %>%
  rowwise() |>
  mutate(bin_naturalbreak = dplyr::case_when(
    varname == 'OperRes' ~ 1,
    original.value >= naturalbreakpoints.1 & original.value < naturalbreakpoints.2 ~ 1,
    original.value >= naturalbreakpoints.2 & original.value < naturalbreakpoints.3 ~ 2,
    original.value >= naturalbreakpoints.3 & original.value <= naturalbreakpoints.4 ~ 3,
    T ~ 99
  )) |> 
  # group_by(varname) %>%
...
```{r show_the_natural_breaks}
breaks_tbl = data_bin |> 
  dplyr::select(varname, starts_with("naturalb")) |> 
  dplyr::distinct() |> 
  dplyr::mutate(naturalbreakpoints.4 = ifelse(naturalbreakpoints.3 == naturalbreakpoints.4 | is.na(naturalbreakpoints.3),NA,naturalbreakpoints.4)) |> 
  dplyr::rename(Variable = varname, Natural_Break_1 = naturalbreakpoints.1, Natural_Break_2 = naturalbreakpoints.2,
                Natural_Break_3 = naturalbreakpoints.3,Natural_Break_4 = naturalbreakpoints.4)

breaks_tbl |> 
  knitr::kable()
  # tidyr::pivot_longer(-varname)
```

### Water Chemistry Variables (pre-binned)
...
calc = read_sf(paste0(my_opts$base_dir,'03_b_PrioritizationModel_Golden_mussel/01_DataCleaning/output/ZQM_RiskAssessment_ssp370_15C_threshold_pH_Adjustment_Subwatershed_golden_mussel.gpkg')) |> 
...
subw_data_bin = subw_data_long %>% 
  ...
  mutate(bin_naturalbreak = as.numeric(cut(original.value, 
                                           breaks = unique(unlist(data))))) |> 
  dplyr::mutate(bin_naturalbreak = tidyr::replace_na(bin_naturalbreak, 0))
...
naturalbreaksfig = ggplot() + 
  ...
  geom_vline(data = subw_natural_break_points %>% 
             ...
             linetype = "dashed",
             col = "transparent") + 
  scale_x_continuous(labels = c("No Risk","Low Risk","Moderate Risk","High Risk")) +
  labs(x = "Variable Values", y = "Number of Subwatersheds") +
  ggtitle("Natural Breaks") + 
...
```{r other_water_variables}
# # Ggplot bar plot showing number of waterbodies in each ZQ mussel risk category, as per the table shown in the methods section.
# data_long |> 
#   dplyr::filter(varname == 'TotalInspections') |> 
#   tidyr::pivot_wider(names_from = varname, values_from = original.value) |> 
#   dplyr::select(TotalInspections, starts_with('water_')) |> 
#   pivot_longer(cols = -TotalInspections) |> 
#   dplyr::count(name,value) |> 
#   ggplot() +
#   geom_col(aes(x = name, y = n, color = value, fill = value), position = position_dodge()) + 
#   facet_wrap( ~ name, nrow = 5, scales = 'free') + 
#   labs(fill = "Risk Level", color = 'Risk Level', x = "Water Chemistry Variable", y = "Number of Waterbodies") + 
#   theme(legend.position = 'bottom') + 
#   scale_fill_manual(values = c("No Risk" = "lightgreen",
#                                "Low Risk" = "yellow",
#                                "Moderate Risk" = "orange",
#                                "High Risk" = "red")) + 
#   scale_colour_manual(values = c("No Risk" = "lightgreen",
#                                "Low Risk" = "yellow",
#                                "Moderate Risk" = "orange",
#                                "High Risk" = "red"))
```
...
# Add on the water chemistry variables.
risk_data = risk_data |> 
  dplyr::left_join(data_long |> 
                     dplyr::select(WATERSH,WATERBO,GNIS_NA,starts_with("water_")) |> 
                     dplyr::distinct())
```

```{r set_up_colour_palette}
my.colours = c("lightgreen","gold","#eb9e4e","#F00320","darkgrey")
```

```{r find_number_of_wbs_in_each_bin_level_for_all_variables}
if(interactive()){
  risk_data |> 
    sf::st_drop_geometry() |> 
    tidyr::pivot_longer(cols = -c(WATERSH,WATERBO,GNIS_NA)) |> 
    dplyr::filter(!is.na(value)) |> 
    dplyr::count(name,value) |> 
    mutate(value = paste0("level_",value)) |> 
    group_by(name) |> 
...
These variables include the number of total inspections, the sum of days fished (2010 and 2023 surveys), and the number of marinas.
...
risk_data_use = risk_data %>% 
  ...
  dplyr::mutate(value = dplyr::case_when(
    # The below inflates the number of waterbodies that are making it into the final list...
    # name %in% c("TotalInspections","Marinas") ~ value*2,
    name %in% c("days_fished_2023_survey") & value == 2 ~ 1.5,
    name %in% c("days_fished_2023_survey") & value == 3 ~ 2,
    T ~ value
  )) |> 
...
Calcium (dissolved and total) was accessed from the provincial Environmental Monitoring System (EMS) database. These values were cleaned of extreme outlier values (i.e. any value less than 0 or greater than 300 mg/L of calcium), spatially matched to water bodies, and averaged at the subwatershed scale (N = 246). Calcium values were binned according to the [2024 DFO Risk Assessment for Zebra and Quagga Mussels](https://publications.gc.ca/collections/collection_2024/mpo-dfo/fs70-5/Fs70-5-2024-008-eng.pdf), i.e. No Risk (\< 8 mg/L), Low (8 to \<15 mg/L), Moderate (15 to \<30 mg/L), and High (30+ mg/L).

Please note that there were many subwatersheds with insufficient data to find a mean calcium level. For those, a risk category of 0 was assigned along with a note that this is due to insufficient data in the 'calcium_data' column.

```{r calcium_subwatershed_figure}

# A similar methodology was used for pH (outliers being any value less than 0 or greater than 16.5 pH). 
# These risk bins were reduced by one (e.g. High -> Moderate) for subwatersheds with pH values outside of optimal conditions for Zebra-Quagga Mussels, i.e. less than 7 or higher than 9.6 pH. In addition, calcium risk levels were reduced by one for subwatersheds with average predicted temperatures below 10 degrees Celsius.
# Apply factor levels to calcium bins.
calc = calc |> 
  dplyr::mutate(calc_bin = factor(calc_bin_pH_temp_adj,
                                  levels = c(
                                    "No Risk (< 1 mg/L)",
                                    "Low (1 to <6 mg/L)",
                                    "Moderate (6 to <12 mg/L)",
                                    "High (12+ mg/L)",
                                    "No Data"
                                  )))
...
ggplot() + 
  ...
  scale_fill_manual(values = c(
    "No Risk (< 1 mg/L)" = 'green',
    "Low (1 to <6 mg/L)" = 'yellow',
    "Moderate (6 to <12 mg/L)" = 'orange',
    "High (12+ mg/L)" = 'red',
    "No Data" = 'grey'
  ))
```
...
### Distinct Federal SARA-listed Species-at-Risk

```{r distinct_federal_sara_species}

# data for mussel-fouled inspections.
risk_data_fed_sara = risk_data %>% 
  st_drop_geometry() %>% 
  #Get rid of empty rows.
  filter(!is.na(distinct_Fed_SARA_listed_SAR)) %>% 
  #Calculate risk column.
  mutate(Risk = distinct_Fed_SARA_listed_SAR) %>% 
  #Round the risk values to nearest whole number.
  mutate(Risk = round(Risk,2)) %>% 
  mutate(Risk = as.character(Risk)) %>% 
  mutate(Risk = replace_na(Risk, "Unknown")) |> 
  dplyr::select(WATERSH, WATERBO, GNIS_NA, distinct_Fed_SARA_listed_SAR, Risk)

waterb_risk = waterb %>% 
  dplyr::select(WATERSH,WATERBO,GNIS_NA) %>% 
  left_join(risk_data_fed_sara) %>% 
  #Replace NA's with a factor called "Unknown"
  mutate(Risk = replace_na(Risk, "Unknown"))

# Table
waterb %>% 
  dplyr::select(distinct_Fed_SARA_listed_SAR) %>% 
  st_drop_geometry() %>% 
  pivot_longer(cols = everything()) %>% 
  rename(Variable = name) %>% 
  filter(!is.na(value),
...
ggplot() +
  ...
  scale_colour_brewer(palette = "RdYlGn", direction = -1) +
  ggthemes::theme_map() + 
  labs(fill = "Binned Values", col = "Binned Values") +
  ggtitle("Distinct Federal SARA sp.",
          subtitle = 'At the Waterbody Scale') +
  theme(
    title = element_text(size = 16)
  )
...
### Distinct Provincial CDC-listed Species-at-Risk

```{r distinct_provincial_cdc_listed_species}
# data for mussel-fouled inspections.
risk_data_cdc_sar = risk_data %>% 
  st_drop_geometry() %>% 
  #Get rid of empty rows.
  filter(!is.na(distinct_CDC_listed_SAR)) %>% 
  #Calculate risk column.
  mutate(Risk = distinct_CDC_listed_SAR) %>% 
  #Round the risk values to nearest whole number.
  mutate(Risk = round(Risk,2)) %>% 
  mutate(Risk = as.character(Risk)) %>% 
  mutate(Risk = replace_na(Risk, "Unknown")) |> 
  dplyr::select(WATERSH, WATERBO, GNIS_NA, distinct_CDC_listed_SAR, Risk)

waterb_risk = waterb %>% 
  dplyr::select(WATERSH,WATERBO,GNIS_NA) %>% 
  left_join(risk_data_cdc_sar) %>% 
  #Replace NA's with a factor called "Unknown"
  mutate(Risk = replace_na(Risk, "Unknown"))

# Table
waterb %>% 
  dplyr::select(distinct_CDC_listed_SAR) %>% 
  st_drop_geometry() %>% 
  pivot_longer(cols = everything()) %>% 
  rename(Variable = name) %>% 
  filter(!is.na(value),
...
risk_data_means = risk_data_use %>% 
  ...
  # full_join(risk_data_suit |> dplyr::select(-Risk)) |> 
  left_join(risk_data_fed_sara |> dplyr::select(-Risk)) |> 
  left_join(risk_data_cdc_sar |> dplyr::select(-Risk)) |> 
  ...
  #Replace NA in the variable group columns with 0s
  mutate(MusselFouled = replace_na(MusselFouled, 0),
         Use = replace_na(Use, 0),
         Highrisk = replace_na(Highrisk, 0),
         # Establishment = replace_na(Establishment, 0),
         SummedDamCapacity = replace_na(SummedDamCapacity, 0),
         distinct_CDC_listed_SAR = replace_na(distinct_CDC_listed_SAR, 0),
         distinct_Fed_SARA_listed_SAR = replace_na(distinct_Fed_SARA_listed_SAR, 0)) %>% 
#Mussel-fouled will be reclassified as 1 or 0 (true or false)
  ...
  mutate(InvasionRisk = Use + MusselFouled + Highrisk + calcium_bin,
         EVRisk = SummedDamCapacity + distinct_CDC_listed_SAR + distinct_Fed_SARA_listed_SAR) %>% 
...
waterb_risk |> 
  st_drop_geometry() |> 
  dplyr::mutate(ENRisk_unbin = distinct_Fed_SARA_listed_SAR + distinct_CDC_listed_SAR + SummedDamCapacity) |> 
...
write_sf(waterb_risk %>% 
         ...
         paste0(my_opts$remote_spatial_data,"Projects/ZQMussels/",my.year," IMDP Final Report/data/spatial/waterbodies_zqm_ALL_risk_estimates_golden_mussel.gpkg"))
...
write_sf(waterb_with_all_vars,
         paste0(my_opts$remote_spatial_data,"Projects/ZQMussels/",my.year," IMDP Final Report/data/spatial/Waterbodies_with_binned_and_original_values_golden_mussel.gpkg"))
```
