...
angsur = sf::read_sf(paste0(lan_root,"2 SCIENCE - Invasives/AIS_R_Projects/CMadsen_Wdrive/shared_data_sets/freshwater_fisheries_society_angler_survey_2022_2023.gpkg"))
...
wat_pH = terra::rast(paste0(onedrive_wd,"../raster/pH_All_masked_krig.tif"))
# Number of waterbodies downstream of waterbody.
# library(fwa.connect)
# fwa_code_tbl = fwa.connect::stream_conn_tbl()
salinity = terra::rast(paste0(onedrive_wd,"../raster/salinity_All_krig.tif"))
calcium = terra::rast(paste0(onedrive_wd,"../raster/Calcium_Dissolved_All_krig.tif "))
...
waterb$salinity = terra::extract(salinity, terra::vect(sf::st_transform(waterb,4326)),
                                 fun = 'mean', na.rm = T)[,2]
waterb$calcium = terra::extract(calcium, terra::vect(sf::st_transform(waterb,4326)),
                                 fun = 'mean', na.rm = T)[,2]
...
waterb = waterb |> dplyr::mutate(salinity = tidyr::replace_na(salinity, 0))
waterb = waterb |> dplyr::mutate(calcium = tidyr::replace_na(calcium,0))
```
...
waterb <- waterb |>
  ...
  dplyr::mutate(water_pH = dplyr::case_when(
    water_pH < 5.8 | water_pH > 9.2 ~ "No Risk",
    water_pH <= 6.5 | water_pH >= 8.8 ~ "Low Risk",
    (water_pH > 6.5 & water_pH < 7.0) | 
      (water_pH >= 8.0 & water_pH < 8.8) ~ "Moderate Risk",
    water_pH >= 7.0 & water_pH <= 8.0 ~ "High Risk",
    TRUE ~ as.character(water_pH)
  )) |>
  ...
  dplyr::mutate(water_temp_interp = dplyr::case_when(
    water_temp_interp < 5 | water_temp_interp > 35 ~ "No Risk",
    water_temp_interp <= 12 | water_temp_interp >= 30 ~ "Low Risk",
    (water_temp_interp > 12 & water_temp_interp <= 18) |
      (water_temp_interp > 26 & water_temp_interp < 30) ~ "Moderate Risk",
    water_temp_interp > 18 & water_temp_interp <= 26 ~ "High Risk",
    TRUE ~ as.character(water_temp_interp)
  )) |>
  ...
  dplyr::mutate(water_chlor_interp = dplyr::case_when(
    water_chlor_interp < 1 | water_chlor_interp > 60 ~ "No Risk",
    water_chlor_interp <= 3 | water_chlor_interp >= 40 ~ "Low Risk",
    (water_chlor_interp > 3 & water_chlor_interp < 10) |
      (water_chlor_interp >= 25 & water_chlor_interp < 40) ~ "Moderate Risk",
    water_chlor_interp >= 10 & water_chlor_interp < 25 ~ "High Risk",
    TRUE ~ as.character(water_chlor_interp)
  )) |>
  ...
  dplyr::mutate(water_dis_ox_interp = dplyr::case_when(
    water_dis_ox_interp < 2 ~ "No Risk",
    water_dis_ox_interp < 4 ~ "Low Risk",
    water_dis_ox_interp < 6 ~ "Moderate Risk",
    water_dis_ox_interp >= 6 ~ "High Risk",
    TRUE ~ as.character(water_dis_ox_interp)
  )) |>
  ...
  dplyr::mutate(water_tot_phos_interp = dplyr::case_when(
    water_tot_phos_interp < 5 | water_tot_phos_interp > 100 ~ "No Risk",
    water_tot_phos_interp <= 15 | water_tot_phos_interp >= 75 ~ "Low Risk",
    (water_tot_phos_interp > 15 & water_tot_phos_interp <= 30) |
      (water_tot_phos_interp > 50 & water_tot_phos_interp < 75) ~ "Moderate Risk",
    water_tot_phos_interp > 30 & water_tot_phos_interp <= 50 ~ "High Risk",
    TRUE ~ as.character(water_tot_phos_interp)
  )) |> 
  dplyr::mutate(salinity = dplyr::case_when(
    salinity > 5 ~ "No Risk",
    salinity > 3 & salinity <= 5 ~ "Low Risk",
    salinity > 1.5 & salinity <= 3 ~ "Moderate Risk",
    salinity >= 0 & salinity <= 1.5 ~ "High Risk",
    TRUE ~ as.character(salinity)
  )) |>

  # -------------------------
  # Calcium (mg/L)
  # -------------------------
  dplyr::mutate(calcium = dplyr::case_when(
    calcium < 3 ~ "No Risk",
    calcium < 6 ~ "Low Risk",
    calcium < 12 ~ "Moderate Risk",
    calcium >= 12 ~ "High Risk",
    TRUE ~ as.character(calcium)
  ))
```
...
openxlsx::write.xlsx(filter_steps,
                     paste0(my_opts$base_dir,"03_PrioritizationModel/output/waterbody_filtering_steps_golden_mussel.xlsx"),
                     overwrite = T)
```
...
openxlsx::write.xlsx(column.names,
                     paste0(my_opts$base_dir,"03_PrioritizationModel/output/column_names_for_waterb_with_data_golden_mussel.xlsx"),
                     overwrite = T)

sf::write_sf(waterb, paste0(my_opts$remote_spatial_data,"Projects/ZQMussels/data/waterb_with_data_golden_mussel.gpkg"))
sf::write_sf(waterb, paste0(my_opts$base_dir,"03_PrioritizationModel/data/waterb_with_data_golden_mussel.gpkg"))
```