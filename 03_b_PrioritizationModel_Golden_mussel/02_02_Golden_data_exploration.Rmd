...
temp_limit = 15 # this is the temp limit for Golden mussel
...
```{r get_monthly_temperatures}

# raster_file_max <- terra::rast(
#   here::here("..", "AIS_hydroclim_data", "output", "max_avg_monthly_temps_bc.tif")
# )
# 
# raster_file_min <- terra::rast(
#   here::here("..", "AIS_hydroclim_data", "output", "min_avg_monthly_temps_bc.tif")
# )
# 
# temp_limit <- 15
# temp_limit_zq <- 10
# 
# 
# raster_file_avg<-(raster_file_max + raster_file_min) / 2
# # fix the rasters, they are *10 to reduce size
# raster_file_avg<- terra::mask(raster_file_avg, bc)
# # Now we will summarise by the watershed for each month
# 
# watershed <- st_transform(watershed, 4326)
# ws_vect <- vect(st_transform(watershed, crs = 4326))
# 
# 
# 
# 
# temp_ext <- terra::extract(
#   raster_file_avg,
#   ws_vect,
#   fun = mean,
#   na.rm = TRUE
...
# 
# temp_long <- temp_ext %>%
#   mutate(WATERSHED_ = watershed$WATERSHED_) %>%  # preserve ID
#   pivot_longer(
#     cols = -c(ID, WATERSHED_),
#     names_to  = "month",
#     values_to = "temp"
#   )
# 
...
#     month = factor(
#       month,
#       levels = names(raster_file_avg),
#       labels = month.name
#     )
#   )
# 
# watershed_long <- watershed %>%
#   left_join(temp_long, by = "WATERSHED_")
# 
# 
# 
# watershed_long <- watershed_long %>%
#   mutate(
...
# watershed_long <- watershed_long %>%
#   mutate(
#     temp_group_zq = case_when(
#       is.na(temp)          ~ NA_character_,
#       temp < temp_limit_zq   ~ paste0("< ", temp_limit_zq),
#       temp >= temp_limit_zq  ~ paste0("≥ ", temp_limit_zq)
#     )
#   )
# 
# 
# # shared temp scale for plots
# shared_temp_limits <- range(
#   c(watershed_long$temp, values(raster_file_avg)),
#   na.rm = TRUE
# )
...
pH_within_ideal_ZQ_range = mean_pH_by_subw |> 
  dplyr::mutate(ideal_conditions = mean_pH >= 6 & mean_pH <= 9)
...
openxlsx::write.xlsx(pH_within_ideal_ZQ_range, 'output/mean_pH_waterbody_matched_within_ideal_Golden_mussel_range_by_subwatershed.xlsx')
...
#     temp < temp_limit ~ paste0("< ", 15),
#     temp >=  temp_limit ~ paste0(">= ", 15),
#     T ~ "NA"
#   ))

# we already did the temperature work, we have it by month.

# watershed_monthly <- watershed_long |>
#   dplyr::select(
#     WATERSHED_,
#     month,
#     temp,
...
# 
# watershed <- watershed |> 
#   dplyr::select(-c( WATERSHED_.y)) |> 
#   rename(WATERSHED_ = WATERSHED_.x)
...
```{r temp_correction_map, fig.height= 6, fig.width=8}
my_plot = ggplot() +
  geom_sf(data = watershed, col = "black", aes(fill = temp_group)) +
  scale_fill_manual(values = c("< 15" = "transparent",
                               ">= 15" = "red")) +
...
ggsave(filename = paste0("./output/TemperatureCorrection_",temp_limit,"_",climate_model,"_RiskMap_Golden_mussel.png"), plot = my_plot, dpi = 150, width = 10, height = 7)

# library(patchwork)
# 
# temp_limit <- 15
# 
# watershed_plot <- watershed |>
#   dplyr::select(WATERSHED_, geometry, starts_with("temp_")) |>
#   tidyr::pivot_longer(
...
#   ) |>
#   dplyr::mutate(
#     month = stringr::str_remove(month, "^temp_"),
#     month = factor(month, levels = month.name),
#     temp_group = dplyr::if_else(
#       temp > temp_limit,
#       paste0("> ", temp_limit),
#       paste0("≤ ", temp_limit)
#     )
...
# library(ggspatial)
# # Precompute labels
# labels <- c(paste0("≤ ", temp_limit), paste0("> ", temp_limit))
# 
# # Assign colors
...
# 
# 
# plot_month_threshold <- function(m) {
#   ggplot(
#     watershed_plot |> dplyr::filter(month == m)
#   ) +
#     geom_sf(
#       aes(fill = temp_group),
#       col = "grey",
...
# 
# 
# plots <- lapply(levels(watershed_plot$month), plot_month_threshold)
# 
# wrap_plots(plots, ncol = 4, guides = "collect") &
...
watershed = watershed |> 
  dplyr::mutate(calc_bin = case_when(
    mean_calcium < 1 ~ 'No Risk (< 1 mg/L)',
    mean_calcium >= 1 & mean_calcium < 6 ~ 'Low (1 to <6 mg/L)',
    mean_calcium >= 6 & mean_calcium < 12 ~ 'Moderate (6 to <12 mg/L)',
    mean_calcium >= 12 ~ 'High (12+ mg/L)',
    is.na(mean_calcium) ~ "No Data"
  )) |> 
  mutate(calc_bin = factor(calc_bin, levels = calc_bin_levels$levels)) |> 
  mutate(calc_bin_n = case_when(
    calc_bin == 'No Risk (< 1 mg/L)' ~ 0,
    calc_bin == 'Low (1 to <6 mg/L)' ~ 1,
    calc_bin == 'Moderate (6 to <12 mg/L)' ~ 2,
    calc_bin == 'High (12+ mg/L)' ~ 3,
    calc_bin == 'No Data' ~ 99 
  ))
```
...
write_sf(watershed,
         paste0("output/ZQM_RiskAssessment_",my_opts$zqm_risk_climate_model,"_",
                temp_limit,"C_threshold_pH_Adjustment_Subwatershed_Golden_mussel.gpkg"))
```
...
ggplot(watershed) +
  ...
  labs(fill = 'Binned Calcium',
       ...
       subtitle = paste0(nrow(watershed[watershed$calc_bin != 'No Data',]), ' of 246 with data; unsuitable pH (<6 or >9) reduced risk by one level (if not already "No Risk")'))

ggsave(filename = 'output/Binned_Mean_Calcium_pH_Adjusted_by_subwatershed_Golden_mussel.png',
       width = 8, height = 8)
```
...
#   dplyr::filter(
#     dplyr::if_any(
#       dplyr::starts_with("temp_"),  # all monthly temp columns
#       ~ . < temp_limit
#     )
#   ) |>
#   dplyr::pull(WATERSHED_)
...
n_waterbodies_with_data <- watershed |>
  dplyr::filter(calc_bin != "No Data") |>
  dplyr::distinct(WATERSHED_) |>   # <-- change to your ID column
  nrow()
...
ggplot(watershed) +
  ...
  labs(fill = 'Binned Calcium',
       ...
       subtitle = paste0(n_waterbodies_with_data, ' of 246 with data; unsuitable pH (<6 or >9) or Temp (<15 C) reduced risk by one level (if not already "No Risk")'))

ggsave(filename = 'output/Binned_Mean_Calcium_pH_and_Temp_Adjusted_by_subwatershed_golden_mussel.png',
       width = 8, height = 8)
```
