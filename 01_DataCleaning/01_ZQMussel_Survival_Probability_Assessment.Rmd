...
wbs = read_sf(paste0(my_opts$remote_spatial_data,"shared_data_sets/summarized_bc_waterbodies_same_gnis_joined.gpkg"))
...
df_sp = st_join(df_sp, wbs, st_intersects)
...
av_df = #rbind(
  df %>% 
    filter(is.na(GNIS_NA)==F) %>% 
    group_by(Watershed, GNIS_NA, MONITORING_LOCATION) %>% 
    summarise(MonitLoc_Mean = mean(RESULT, na.rm=T)) %>%
    summarise(Waterbody_Mean = mean(MonitLoc_Mean,na.rm=T)) %>% 
...
#) %>% 
  ...
  left_join(df %>% 
              filter(is.na(GNIS_NA)==F) %>%
              dplyr::select(Watershed, GNIS_NA) %>%
              distinct() %>%
              group_by(Watershed) %>%
              summarise(wbs_with_data = n())
  ) %>%
  # left_join(df %>% 
  #             filter(is.na(RiverNumber)==F) %>%
  ...
  left_join(df %>% 
              ...
              distinct() %>%
              group_by(Watershed) %>%
              summarise(wbs_monit_locs = n())) %>% 
  mutate(across(everything(), \(x) replace_na(x, 0))) %>% 
  group_by(Watershed) %>%
  mutate(Waterbody_Number = wbs_with_data,
         Monitoring_Loc_Number = wbs_monit_locs) %>%
  ungroup() %>% 
...
watdat = watershed %>% 
  ...
  left_join(av_df) %>% 
  mutate(across(wbs_with_data:Monitoring_Loc_Number, \(x) replace_na(x,0))) %>% 
  mutate(across(ends_with("_bin"), \(x) replace_na(x,"No Data")))
