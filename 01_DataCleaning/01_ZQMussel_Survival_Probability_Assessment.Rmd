...
# # Water Temperature Data (Water Survey Canada)
# bc_stations = hy_stations(prov_terr_state_loc = 'BC')
# 
# sed_samples_bc = hy_sed_samples(station_number = bc_stations$STATION_NUMBER) |> 
#   dplyr::filter(!is.na(TEMPERATURE))
#   
# sed_samples_bc |> 
#   dplyr::count(STATION_NUMBER, TEMPERATURE, sort = T) |> 
#   dplyr::count(STATION_NUMBER)
# 
# dplyr::mutate(SAMPLING_VERTICAL_LOCATION = as.numeric(SAMPLING_VERTICAL_LOCATION),
#                 TEMPERATURE = as.numeric(TEMPERATURE)) |> 
#   dplyr::select(STATION_NUMBER, Date, TEMPERATURE, SAMPLING_VERTICAL_LOCATION)
# 
# bc_stations_sf = st_as_sf(bc_stations, coords = c('LONGITUDE','LATITUDE'), crs = 4326)
# 
# # Add the temperature on to the station sf
# bc_stations_w_temp = bc_stations_sf |> 
#   dplyr::left_join(sed_samples_bc |> 
#                      dplyr::group_by(STATION_NUMBER) |> 
#                      dplyr::reframe(TEMPERATURE = mean(TEMPERATURE,na.rm=T)))
# 
# bc_stations_w_temp |> 
#   dplyr::filter(!is.na(TEMPERATURE))
# 
# ggplot() + 
#   geom_sf(data = bc_shp) + 
#   geom_sf(data = bc_stations_w_temp,
#           aes(col = TEMPERATURE)) 
...
# Quick plot of water temperature. Can we interpolate it?
# terra::plot(terra::rast("C:/Users/CMADSEN/OneDrive - Government of BC/data/raster/Temperature_All_masked_krig.tif"))
```

```{r brief_aside_does_latitude_elevation_and_air_temp_predict_water_temp}
# 
# temp_by_depth = ems_temp |> 
#   dplyr::group_by(UPPER_DEPTH) |> 
#   dplyr::reframe(mean_temp = mean(RESULT,na.rm=T))
# 
# # ggplot(ems_temp) + geom_histogram(aes(UPPER_DEPTH))
# 
# # Average ems temperature by season and location.
# # Seasons: winter == 1, spring == 2, summer == 3, fall == 4. 
# # Winter: Dec, Jan, Feb; so on and so forth.
# library(data.table)
# 
# bc = bcmaps::bc_bound() |> dplyr::summarise() |> terra::vect()
# 
# ems_temp = as.data.table(ems_temp)
# 
# ems_temp_w_date = ems_temp[,my_date := lubridate::ymd_hms(COLLECTION_START)]
# 
# # ggplot(ems_temp_w_date) + geom_histogram(aes(my_date))
# 
# ems_temp_w_date$season = as.numeric(ems_temp_w_date[,.(season = format(my_date,'%m'))]$season)
# 
# ems_temp_w_date = ems_temp_w_date[, season := fcase(
#   season %in% c(12,1:2), 'winter',
#   season %in% c(3:5), 'spring',
#   season %in% c(6:8), 'summer',
#   season %in% c(9:11), 'fall'
# )]
# 
# # Average by location.
# mean_t_by_coords = ems_temp_w_date[,.(av_temp = mean(RESULT)), by = .(LATITUDE,LONGITUDE)][!is.na(LATITUDE),]
# mean_t_by_c_sf = sf::st_as_sf(mean_t_by_coords, coords = c("LONGITUDE","LATITUDE"), crs = 4326)
# 
# # Average by location and season.
# mean_t_by_season_coords = ems_temp_w_date[,.(av_temp = mean(RESULT)), by = .(season,LATITUDE,LONGITUDE)][!is.na(LATITUDE),]
# meant_by_s_c_sf = sf::st_as_sf(mean_t_by_season_coords, coords = c("LONGITUDE","LATITUDE"), crs = 4326)
# mean_t_by_winter_sf = sf::st_as_sf(mean_t_by_season_coords[season == 'winter',], coords = c("LONGITUDE","LATITUDE"),crs = 4326)
# 
# mean_t_by_summer_sf = sf::st_as_sf(mean_t_by_season_coords[season == 'summer',], coords = c("LONGITUDE","LATITUDE"),crs = 4326)
# 
# # Get elevation
# library(elevatr)
# el = elevatr::get_elev_point(locations = mean_t_by_c_sf)
# sf::write_sf(el, "ems_water_temp_data_averaged_by_location_with_elevation_data_added.gpkg")
# 
# # Add mean annual air temperature from bioclim
# air_temp = terra::rast("C:/Users/CMADSEN/OneDrive - Government of BC/data/CNF/climate/wc2.1_10m/wc2.1_10m_bio_1.tif")
# 
# el$mean_air_temp = terra::extract(air_temp, terra::vect(mean_t_by_c_sf))[,2]
# 
# el$lat = sf::st_coordinates(el)[,2]
# 
# # First test: can we predict water temp with air temp, latitude, and elevation?
# annual_model = lm(av_temp ~ mean_air_temp + lat + elevation, data = el)
# summary(annual_model)
# annual_model_glm = glm(av_temp ~ mean_air_temp + lat + elevation, data = el)
# summary(annual_model_glm)
# # Terrible!
# 
# # Maybe we can predict max and min temps?
# air_temp_mean_of_warmest_q = terra::rast("C:/Users/CMADSEN/OneDrive - Government of BC/data/CNF/climate/wc2.1_10m/wc2.1_10m_bio_10.tif")
# air_temp_mean_of_coldest_q = terra::rast("C:/Users/CMADSEN/OneDrive - Government of BC/data/CNF/climate/wc2.1_10m/wc2.1_10m_bio_11.tif")
# 
# ems_dat_winter = ems_temp_w_date |> dplyr::filter(season == 'winter')
# ems_dat_summer = ems_temp_w_date |> dplyr::filter(season == 'summer')
# 
# # Average by location
# mean_t_by_coords_winter = ems_dat_winter[,.(av_temp = mean(RESULT)), by = .(LATITUDE,LONGITUDE)][!is.na(LATITUDE),]
# mean_t_by_coords_summer = ems_dat_summer[,.(av_temp = mean(RESULT)), by = .(LATITUDE,LONGITUDE)][!is.na(LATITUDE),]
# 
# winter_el = elevatr::get_elev_point(locations = sf::st_as_sf(mean_t_by_coords_winter, coords = c("LONGITUDE","LATITUDE"), crs = 4326))
# 
# summer_el = elevatr::get_elev_point(locations = sf::st_as_sf(mean_t_by_coords_summer, coords = c("LONGITUDE","LATITUDE"), crs = 4326))
# 
# winter_el$lat = sf::st_coordinates(winter_el)[,2]
# summer_el$lat = sf::st_coordinates(summer_el)[,2]
# 
# winter_el$mean_air_temp = terra::extract(air_temp, terra::vect(sf::st_as_sf(mean_t_by_coords_winter, coords = c("LONGITUDE","LATITUDE"), crs = 4326)))[,2]
# 
# summer_el$mean_air_temp = terra::extract(air_temp, terra::vect(sf::st_as_sf(mean_t_by_coords_summer, coords = c("LONGITUDE","LATITUDE"), crs = 4326)))[,2]
# 
# annual_model = lm(av_temp ~ mean_air_temp + lat + elevation, data = winter_el)
# summary(annual_model)
# annual_model_glm = glm(av_temp ~ mean_air_temp + lat + elevation, data = winter_el)
# summary(annual_model_glm)
# 
# annual_model = lm(av_temp ~ mean_air_temp + lat + elevation, data = summer_el)
# summary(annual_model)
# annual_model_glm = glm(av_temp ~ mean_air_temp + lat + elevation, data = summer_el)
# summary(annual_model_glm)
