...
final_waterbody_list = final_waterbody_list |> 
  dplyr::mutate(`Veliger Sampling Frequency` = ifelse(InvasionRisk_bin == 3 | Highrisk == 3, "Bi-weekly", "Monthly"))

final_waterbody_list = final_waterbody_list |> 
  dplyr::arrange(`Veliger Sampling Frequency`, InvasionRisk_bin, Use)
...
last_year = get_wb_list_from_protocol_doc(
  ...
  mutate(region_wb = paste0(Region,'-',Waterbody)) |> 
  rowwise() |> 
  mutate(only_on_last_years_list = !region_wb %in% unique(final_waterbody_list$region_wb))
  
final_waterbody_list = final_waterbody_list |> 
  rowwise() |> 
  dplyr::mutate(new_on_this_years_list = !region_wb %in% last_year$region_wb) |> 
  ungroup()
...
# # Figure out which subwatershed each of these new waterbodies is in.
# for(i in 1:nrow(waterbodies_added_this_year)){
#   the_region = waterbodies_added_this_year$Region[i]
#   the_wb_name = waterbodies_added_this_year$Waterbody[i]
#   
#   bcdc_query_geodata('freshwater-atlas-lakes') |> 
#     filter(GNIS_NAME_1 == the_wb_name) |> 
#     collect()
#   
# }
