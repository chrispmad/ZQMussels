...
```{r read_in_data}
# Read RDS.
dat = read_rds(paste0("C:/Users/JPHELAN/Documents/mussel_model/data_for_making_shortlist.rds"))
# if(interactive()){
...
write.csv(queried_wbs, "./output/qeuried_wbs.csv")
...
past_sample_protocols = sample_protocol_2021 |> 
  ...
  bind_rows(sample_protocol_2025 |> mutate(Lat = as.numeric(Lat), Long = as.numeric(Long)) |> 
              mutate(sampling_year = 2025))
...
```{r remove_LowUse_and_LowCalcium}

### we will also filter out any waterbodies that have calcium < 2 and Use < 2

# waterb_risk_high = waterb_risk |>
#  filter(Use >= 2 & calcium_bin >= 2 | 
#           GNIS_NA %in% unique(waterb_with_any_mf$GNIS_NA) & WATERSH %in% unique(waterb_with_any_mf$WATERSH) | keep_me_always)
# 
# waterb_risk_low <- waterb_risk |>
#   st_drop_geometry() |>
#   anti_join(
#     waterb_risk_high |>
#       st_drop_geometry(),
...
#   )
# 
# waterb_risk_low = waterb_risk_low |> 
#   st_drop_geometry()
# 
# write.xlsx(waterb_risk_low, "./output/low_use_low_calcium_waterbodies.xlsx")


# 2.  Keep waterbodies with a binned value of the 'Use' category > 2 combined with binned calcium > 2 results in (`r nrow(waterb_risk)` waterbodies retained)
```
