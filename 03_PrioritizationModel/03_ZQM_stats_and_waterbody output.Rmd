...
![](data/ZQ_Mussel_water_chem_vas_suitability_table.png)
...
$$I_w = (\frac{T_w+A_w+M_w}{n}) + (\frac{H_w-O_w}{n}) + (\frac{MF_w}{n}) + {C_w} + (\frac{T_w + pH_w + Phos_w + Chlor_w + DO_w}{n})$$

Where *Iw* represents the invasion / establishment risk estimate of a waterbody *w*. *T* refers to the total number of watercraft inspections of watercraft headed to waterbody *w*, *A* to angling effort (sum of days fished), *M* to the number of marinas in waterbody *w*, *H* to high-risk inspections and *O* refers to operational restrictions. *MF* to mussel-fouled inspections, *C* stands for the subwatershed-level calcium level (calculated using Environment Monitoring System data), *T* stands for water temperature, *pH* stands for water pH, *Phos* stands for total aquatic phosphorous, *Chlor* stands for aquatic chlorophyll, and *DO* stands for dissolved oxygen. *O* represents a risk reduction, and so it is subtracted from the overall risk estimate.
...
data_long = waterb %>%
  ...
  pivot_longer(all_of(c(all.my.vars[!all.my.vars %in% WaterSuitabilityVars],"total_HR")), names_to = "varname", values_to = "original.value") %>% 
...
calc = read_sf(paste0(my_opts$base_dir,'01_DataCleaning/output/ZQM_RiskAssessment_ssp370_10C_threshold_pH_Adjustment_Subwatershed.gpkg')) |> 
  ...
  mutate(ZQ_CalcRisk = ifelse(calc_bin_n == 99, NA, calc_bin_n))
...
```{r}
risk_data_suit = risk_data |> 
  dplyr::select(WATERSH,WATERBO,GNIS_NA,WaterSuitabilityVars) %>% 
  st_drop_geometry() %>% 
  pivot_longer(cols = all_of(WaterSuitabilityVars)) %>% 
  # Find numeric levels of factors
  dplyr::mutate(value_as_num = as.numeric(value)) |> 
  dplyr::group_by(WATERSH,WATERBO,GNIS_NA) |>
  mutate(Establishment = mean(value_as_num)) %>% 
  dplyr::ungroup() |> 
  #Get rid of the original variables, keeping the mean values.
  dplyr::select(-name,-value,-value_as_num) %>% 
  distinct() %>% 
  #Replace NA in the 3 variable group columns with 0s
  mutate(Establishment = replace_na(Establishment, 0)) %>% 
  mutate(Establishment = round(Establishment,0)) |> 
  #Calculate risk column.
  mutate(Risk = Establishment) %>% 
  #Bin the risk values.
  mutate(Risk = round(Risk,2))
  # Round the Establishment risk to the nearest integer.
  

waterb_risk = waterb %>% 
  dplyr::filter(!is.na(TotalInspections)) |>
  dplyr::select(WATERSH,WATERBO,GNIS_NA) %>% 
  left_join(risk_data_suit) %>% 
  mutate(Risk = as.character(Risk)) %>% 
  #Replace NA's with a factor called "Unknown"
  dplyr::mutate(Risk = factor(Risk, levels = c("1","2","3")))

# Simple count table of the factor levels by water chemistry variable
waterb %>% 
  dplyr::select(all_of(WaterSuitabilityVars)) %>% 
  st_drop_geometry() %>% 
  pivot_longer(cols = everything()) |> 
  dplyr::count(name,value)
...
ggplot() +
  ...
  scale_colour_brewer(palette = "RdYlGn", direction = -1) +
  ggthemes::theme_map() + 
  ggtitle("Establishment Variables") +
  theme(
    title = element_text(size = 16)
  )
```
...
#Mussel-fouled will be reclassified as 1 or 0 (true or false)
  ...
  #Calculate risk column.
  mutate(InvasionRisk = Use + MusselFouled + Highrisk + calcium_bin + Establishment,
