...
# connectivity$number_roads = NULL
# 
# waterb = waterb |>
#   dplyr::left_join(connectivity)

# # Number of inflows per waterbody?
# library(fwa.connect)
# inflow_tbl = fwa.connect::stream_conn_tbl

# # DFO calcium map
# dfo_calc_estab_risk = terra::rast(paste0(my_opts$base_dir,'03_PrioritizationModel/data/DFO_2024_ZQM_Calcium_Establishment_Potential.tif'))
```
...
* Five categories of variables were defined: 
  + 1. Use (Total Watercraft Inspections, Days fished (2023 survey), number of marinas, and distance to nearest heighway)
  ...
  + 5. Water Abiotic Variables (pH, temperature, total phosphorous, chlorophyll, dissolved oxygen)
  + 5. Connectivity (number of waterbodies in subwatershed)
...
$$I_w = (\frac{T_w+A_w+M_w}{n}) + (\frac{H_w-O_w}{n}) + (\frac{MF_w}{n}) + {C_w} + (\frac{T_w + pH_w + Phos_w + Chlor_w + DO_w}{n}) + (\frac{NumLakes_w}{n})$$

Where _Iw_ represents the invasion / establishment risk estimate of a waterbody *w*. *T* refers to the total number of watercraft inspections of watercraft headed to waterbody *w*, *A* to angling effort (sum of days fished), *M* to the number of marinas in waterbody *w*, *H* to high-risk inspections and *O* refers to operational restrictions. *MF* to mussel-fouled inspections, *C* stands for the subwatershed-level calcium level (calculated using Environment Monitoring System data), *T* stands for water temperature, *pH* stands for water pH, *Phos* stands for total aquatic phosphorous, *Chlor* stands for aquatic chlorophyll, *DO* stands for dissolved oxygen, and *NumLakes* stands for the number of connected lakes (calculated by hydrological subwatershed). *O* represents a risk reduction, and so it is subtracted from the overall risk estimate.

$$EV_w = (D_w) + (FSARA_w) + (PSAR_w)$$
Where _EV_w_ represents the economic value risk estimate of a waterbody. *D* refers to the summed dam capacity at the waterbody scale, *FSARA* refers to the number of distinct species of aquatic, federally SARA-listed species-at-risk, and *PSAR* refers to the number of distinct species of aquatic, provincially listed by the Conservation Data Centre as species-at-risk, all at the waterbody scale.
...
all.my.vars = c("TotalInspections",
                ...
                "water_pH",
                "number_of_lakes_in_subwatershed",
                "SummedDamCapacity",
...
Connectivityvars = c("number_of_lakes_in_subwatershed")
...
Calcium (dissolved and total) was accessed from the provincial Environmental Monitoring System (EMS) database. These values were cleaned of extreme outlier values (i.e. any value less than 0 or greater than 300 mg/L of calcium), spatially matched to water bodies, and averaged at the subwatershed scale (N = 246). A similar methodology was used for pH (outliers being any value less than 0 or greater than 16.5 pH). Calcium values were binned according to the [2024 DFO Risk Assessment for Zebra and Quagga Mussels](https://publications.gc.ca/collections/collection_2024/mpo-dfo/fs70-5/Fs70-5-2024-008-eng.pdf), i.e. No Risk (< 8 mg/L), Low (8 to <15 mg/L), Moderate (15 to <30 mg/L), and High (30+ mg/L). These risk bins were reduced by one (e.g. High -> Moderate) for subwatersheds with pH values outside of optimal conditions for Zebra-Quagga Mussels, i.e. less than 7 or higher than 9.6 pH. In addition, calcium risk levels were reduced by one for subwatersheds with average predicted temperatures below 10 degrees Celsius.
...
calc = calc |> 
  dplyr::mutate(calc_bin_pH_temp_adj = factor(calc_bin_pH_temp_adj,
                                              levels = c(
                                                "No Risk (< 8 mg/L)",
                                                "Low (8 to <15 mg/L)",
...
risk_data_suit = risk_data %>% 
  ...
  #Add a column that groups rows by their variable of focus.
  mutate(Grouper = case_when(
    name %in% WaterSuitabilityVars ~ "Establishment")) %>% 
  group_by(WATERSH,WATERBO,GNIS_NA,Grouper) %>% 
  #Calculate the mean value for each of the 3 groups of variables.
  mutate(MeanValue = mean(value)) %>% 
  dplyr::ungroup() |> 
  ...
  pivot_wider(names_from = Grouper, values_from = MeanValue) %>% 
  #Replace NA in the 3 variable group columns with 0s
  mutate(Suitability = replace_na(Use, 0)) %>% 
  #Calculate risk column.
  mutate(Risk = Suitability) %>% 
  #Bin the risk values.
  mutate(Risk = round(Risk,2))
...
The number of connected waterbodies within each subwatershed.

```{r}
risk_data_connectivity = risk_data %>% 
  # dplyr::filter(!is.na(TotalInspections)) |> 
  dplyr::select(WATERSH,WATERBO,GNIS_NA,Connectivityvars) %>% 
  st_drop_geometry() %>% 
  pivot_longer(cols = all_of(Connectivityvars)) %>% 
  #Get rid of empty rows and 0s.
  filter(!is.na(value),
         value > 0) %>% 
  mutate(Grouper = case_when(
    name %in% Connectivityvars ~ "Connectivity")) %>% 
  group_by(WATERSH,WATERBO,GNIS_NA,Grouper) %>% 
  #Calculate the mean value for each of the 3 groups of variables.
  mutate(MeanValue = mean(value)) %>% 
  dplyr::ungroup() |> 
  ...
  pivot_wider(names_from = Grouper, values_from = MeanValue) %>% 
  #Replace NA in the 3 variable group columns with 0s
  mutate(Connectivity = replace_na(Connectivity)) %>% 
  #Calculate risk column.
  mutate(Risk = Connectivity) %>% 
  #Bin the risk values.
  mutate(Risk = round(Risk,2)) |> 
  dplyr::ungroup()

waterb_connectivity = waterb %>% 
  # dplyr::filter(!is.na(TotalInspections)) |>
  dplyr::select(WATERSH,WATERBO,GNIS_NA) %>% 
  left_join(risk_data_connectivity) %>% 
  mutate(Risk = as.character(Risk)) %>% 
  #Replace NA's with a factor called "Unknown"
  mutate(Risk = replace_na(Risk, "Unknown")) |> 
  dplyr::mutate(Risk = as.numeric(Risk)) |> 
...
# knitr::kable(
waterb %>% 
  dplyr::select(all_of(Connectivityvars)) %>% 
  st_drop_geometry() %>% 
  pivot_longer(cols = everything()) %>% 
  rename(Variable = name) %>% 
  filter(!is.na(value),
...
# #```{r}
# risk_data[!is.na(risk_data$distance_to_nearest_highway),] |> 
#   # Flip bin levels?
#   dplyr::mutate(distance_to_nearest_highway = dplyr::case_when(
#     distance_to_nearest_highway == 3 ~ 1,
#     distance_to_nearest_highway == 1 ~ 3,
#     T ~ distance_to_nearest_highway
#   )) |> 
#   dplyr::mutate(distance_to_nearest_highway = factor(distance_to_nearest_highway, levels = c(1,2,3,NA))) |> 
#   ggplot() + 
#   geom_sf(data = bc, fill = "lightgrey") +
#   geom_sf(aes(fill = distance_to_nearest_highway, col = distance_to_nearest_highway)) +
#   scale_fill_brewer(palette = "RdYlGn", direction = -1) +
#   scale_colour_brewer(palette = "RdYlGn", direction = -1) +
#   ggthemes::theme_map() + 
#   ggtitle("Distance to Nearest Highway") +
#   theme(
#     title = element_text(size = 16)
#   ) + 
#   labs(fill = 'Binned\nDistance to \nHighway',
...
# #```{r}
# risk_data[!is.na(risk_data$wbs_downstream),] |> 
#   dplyr::mutate(wbs_downstream = factor(wbs_downstream, levels = c(NA,1,2,3))) |> 
#   ggplot() + 
#   geom_sf(data = bc, fill = "lightgrey") +
#   geom_sf(aes(fill = wbs_downstream, col = wbs_downstream)) +
#   scale_fill_brewer(palette = "RdYlGn", direction = -1) +
#   scale_colour_brewer(palette = "RdYlGn", direction = -1) +
#   ggthemes::theme_map() + 
#   ggtitle("Number of Downstream Waterbodies") +
#   theme(
#     title = element_text(size = 16)
#   ) + 
#   labs(fill = 'Binned\nWaterbodies\nDownstream',
...
risk_data_means = risk_data_use %>% 
  ...
  full_join(risk_data_connectivity |> dplyr::select(-Risk)) |> 
  ...
  mutate(MusselFouled = replace_na(MusselFouled, 0),
         ...
         Suitability = replace_na(Suitability, 0),
...
#Mussel-fouled will be reclassified as 1 or 0 (true or false)
  ...
  #Calculate risk column.
  mutate(InvasionRisk = Use + MusselFouled + Highrisk + calcium_bin + Suitability + Connectivity,
