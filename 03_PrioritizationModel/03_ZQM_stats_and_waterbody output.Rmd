...
* Five categories of variables were defined: 
  + 1. Use (Total Watercraft Inspections, Angler use (2010 survey), Days fished (2023 survey), and number of marinas) (number of abutting streams and number of nearby roads removed for this iteration)
  ...
  + 5. Connectivity (number of abutting streams and number of roads within 15 meters)
...
$$I_w = (\frac{T_w+A_w+M_w}{n_w}) + (\frac{H_w-O_w}{n_w}) + (\frac{MF_w}{n_w}) + (C_s) + (\frac{SA_w+AR_w}{n_w})$$

Where _Iw_ represents the invasion / establishment risk estimate of a water body *w*. *T* refers to the total number of watercraft inspections of watercraft headed to waterbody *y*, *A* to angling effort (sum of angling days), *M* to the number of marinas in waterbody *w*, *H* to high-risk inspections and *O* refers to operational restrictions. *MF* to mussel-fouled inspections, *C* stands for the subwatershed-level calcium level (calculated using Environment Monitoring System data), *SA* stands for the number of streams abutting the waterbody, and *AR* stands for the number of potential access roads near the waterbody. *O* represents a risk reduction, and so it is subtracted from the overall risk estimate.

$$EV_w = (D_w) + (FSARA_w) + (PSAR_w)$$
Where _EV_w_ represents the economic value risk estimate of a water body. *D* refers to the summed dam capacity at the waterbody scale, *FSARA* refers to the number of distinct species of aquatic, federally SARA-listed species-at-risk, and *PSAR* refers to the number of distinct species of aquatic, provincially listed by the Conservation Data Centre as species-at-risk, all at the waterbody scale.
...
all.my.vars = c("Marinas","Sum.of.days.fished_2010_survey",
...
Usevars = c("TotalInspections","Sum.of.days.fished_2010_survey","days_fished_2023_survey","Marinas")
Connectivityvars = c("number_streams","number_roads")
...
The number of abutting streams, and the number of nearby access roads for each waterbody.
...
#### Number of Tributary Waterbodies
```{r}
risk_data[!is.na(risk_data$number_streams),] |> 
  dplyr::mutate(number_streams = factor(number_streams, levels = c(NA,1,2,3))) |> 
  ggplot() + 
  geom_sf(data = bc, fill = "lightgrey") +
  geom_sf(aes(fill = number_streams, col = number_streams)) +
  scale_fill_brewer(palette = "RdYlGn", direction = -1) +
  scale_colour_brewer(palette = "RdYlGn", direction = -1) +
  ggthemes::theme_map() + 
  ggtitle("Number of Tributary Streams/Rivers") +
  theme(
    title = element_text(size = 16)
  ) + 
  labs(fill = 'Binned\nNumber of\nStreams',
...
```{r}
risk_data[!is.na(risk_data$number_roads),] |> 
  dplyr::mutate(number_roads = factor(number_roads, levels = c(NA,1,2,3))) |> 
  ggplot() + 
  geom_sf(data = bc, fill = "lightgrey") +
  geom_sf(aes(fill = number_roads, col = number_roads)) +
  scale_fill_brewer(palette = "RdYlGn", direction = -1) +
  scale_colour_brewer(palette = "RdYlGn", direction = -1) +
  ggthemes::theme_map() + 
  ggtitle("Number of Tributary Streams/Rivers") +
...
#Mussel-fouled will be reclassified as 1 or 0 (true or false)
  ...
  #Calculate risk column.
  mutate(InvasionRisk = Use + MusselFouled + Highrisk + calcium_bin + Connectivity,
