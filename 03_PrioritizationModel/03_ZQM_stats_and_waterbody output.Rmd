...
sar = sf::read_sf(paste0(my_opts$base_dir,'../species_at_risk_visualizer/app/www/species_ecosystems_at_risk_publically_available.gpkg')) |> 
...
subw_w_dat = subw |> 
  left_join(calc |> 
              dplyr::rename(WATERSHED_ = Watershed) |> 
              sf::st_drop_geometry() |> 
              dplyr::select(WATERSHED_,ZQ_CalcRisk)) #|> 
  # left_join(sar_by_subw |> 
  #             dplyr::rename(WATERSHED_ = WATERSH) |> 
  #             sf::st_drop_geometry() |> 
  #             dplyr::select(WATERSHED_,distinct_SAR)) |> 
  # left_join(dams |> 
  #             dplyr::rename(WATERSHED_ = WATERSH) |> 
  #             sf::st_drop_geometry() |> 
  #             dplyr::group_by(WATERSHED_) |> 
  #             dplyr::summarise(SummedDamCapacity = sum(DmCpcty)) |> 
  #             dplyr::select(WATERSHED_,SummedDamCapacity))
# subw_wout_calc = subw %>% 
...
ggplot() + 
  ...
  geom_sf(data = calc, aes(fill = ZQ_CalcRisk)) + 
...
if(!dir.exists(paste0(my_opts$remote_spatial_data,"Projects/ZQMussels/",my_opts$year," IMDP Final Report/data/spatial"))) { 
  dir.create(paste0(my_opts$remote_spatial_data,"Projects/ZQMussels/",my_opts$year," IMDP Final Report/data/spatial"), recursive = T)
}

write_sf(waterb_risk %>% 
           filter(InvasionRisk != "Unknown"),
         paste0(my_opts$remote_spatial_data,"Projects/ZQMussels/",my_opts$year," IMDP Final Report/data/spatial/waterbodies_zqm_ALL_risk_estimates.gpkg"))
```
