...
```{r query_EMS_database_for_calcium}

```

```{r filter_EMS_data_a_la_DFO_methods}

```

```{r interpolate_calcium_across_BC}
...
#with a numeric 1.
waterb = waterb %>% 
  mutate(boat.launch_2010_survey = replace(boat.launch_2010_survey, boat.launch_2010_survey == "Y", 1)) %>% 
  mutate(boat.launch_2010_survey = as.numeric(boat.launch_2010_survey))

# Add the boating_sites data from the BC Data Catalogue. NOTE: Only 4 points overlap with our lakes! What the heck.
# Don't join for now.
...
waterb = waterb %>% 
  mutate(Campgrounds = sum(Campgrounds, Campsites_N, na.rm=T),
         BoatLaunches = sum(boat.launch_2010_survey,BoatLaunchRecCounter,Boat_Launches_N,na.rm=T)) %>% select(-Campsites_N,-BoatLaunchRecCounter,-Boat_Launches_N,-boat.launch_2010_survey) %>% 
