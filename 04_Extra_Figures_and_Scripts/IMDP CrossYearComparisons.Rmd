...
dat2019 = openxlsx::read.xlsx("C:/Users/CMADSEN/Downloads/LocalRWork/output/IMDP_2019_HighRisk_DeconOrdIss_DeconPerf.xlsx")
...
decondat = dat %>%
  ...
  mutate(Performed = replace_na(Performed,0)) %>% 
  mutate(Total = sum(Ordered, Performed)) %>% 
  pivot_longer(cols = c(Ordered,Performed)) %>% 
  arrange(desc(Total))
...
decondat %>% 
  ...
  geom_text(aes(label = value, x = Station, y = value+2), 
            colour = "black",
            position = position_dodge2(1, preserve = "single")) +
  labs(y = "Number of Decontaminations Performed", fill = "Decontamination") +
  theme_minimal() +
  theme(panel.grid.minor = element_blank(),
        text = element_text(size = 13),
        axis.text.x = element_text(size=13,angle = 60,hjust=1)) +
