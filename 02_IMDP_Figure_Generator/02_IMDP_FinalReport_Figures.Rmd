...
```{r 2025_station_changes}
# This should be elsewhere, but here we are.
# Douglas is Douglas Crossing, for now. It gets changed later
...
lw <- dat |>
  ...
  filter(
    str_detect(
      Station,
      "Lower Mainland.*(Outreach|Scheduled Inspection|Roving)|^Lower Mainland$"
    ),
    !str_detect(Station, "Douglas|Huntington")
  )

p_insp <- lw |>
  mutate(the.month = month(TimeOfInspection, label = TRUE, abbr = FALSE)) |>
  filter(!(the.month %in% c("March","November") & Year == 2024)) |>
  group_by(the.month, Station) |>
  summarise(Number_Insp = n(), .groups = "drop") |>
  ggplot(aes(x = the.month, y = Number_Insp, fill = Station)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6) +
  geom_text(
    aes(label = Number_Insp),
...
p_effort <- lw |>
  ...
  mutate(the.month = month(TimeOfInspection, label = TRUE, abbr = FALSE)) |>
  filter(!(the.month %in% c("March","November") & Year == 2024)) |>
  select(the.month, Station, Shift_ID, Shift_hours) |>
  distinct() |>
  filter(Shift_hours > 0) |>
  group_by(the.month, Station) |>
  summarise(TotalEffort = sum(Shift_hours), .groups = "drop") |>
  ggplot(aes(x = the.month, y = TotalEffort, fill = Station)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6) +
  geom_text(
    aes(label = round(TotalEffort, 0)),
    position = position_dodge(width = 0.7),
    vjust = -0.3,
    size = 3
  ) +
...
p5 <- ggarrange(p_insp, p_effort, ncol = 2, nrow = 1)
...
if(!interactive()) {
  ...
  ggsave(paste0(my.external.output.folder,'/GIS Maps and Excel Figures/ExcelFigures/Lower_Mainland_Outreach_Sch.jpg'), p5, width = 7.6, height = 6.4, dpi = 300)
}


# if(!interactive()) {
#   # tmap_save(tm = map_1_tmap, filename = paste0(my.external.output.folder,'/GIS Maps and Excel Figures/ExcelFigures/Map1_Stations_by_Type.jpg'), width = 7.6, height = 6.4, dpi = 300)
#   ggsave(paste0(my.external.output.folder,'/GIS Maps and Excel Figures/ExcelFigures/Penticton_Outreach_Sch.jpg'), p5, width = 7.6, height = 6.4, dpi = 300)
# }


### Okay lets fix some names

dat = dat |> 
  mutate(Station = case_when(
      Station == "Lower Mainland Roving - Douglas"      ~ "Douglas Crossing",
      Station == "Lower Mainland - Douglas"      ~ "Douglas Crossing",
      Station == "Lower Mainland - Huntington"      ~ "Sumas (Huntington)",
      Station == "Lower Mainland - Scheduled Inspection"      ~ "Lower Mainland",
      Station == "Lower Mainland Roving - Outreach"      ~ "Lower Mainland",
...
blank_comments <- dat |>
  filter(str_detect(Station, "Other|Emergency|Project|Lower Mainland - Scheduled Inspection|Lower Mainland Roving - Scheduled Inspection|Lower Mainland Roving - Douglas|Lower Mainland - Douglas"))


### These are the Peace Arch 
# dfo_comments = dat |> 
#   filter(str_detect(Shift_Start_Comment, "DFO")|
#          str_detect(Shift_End_Comment, "DFO")) |> 
#   filter(as_date(TimeOfInspection) %in% as.Date(c(
#     "2025-07-04",
#     "2025-07-05",
#     "2025-07-06",
#     "2025-07-18",
...
#     
#   ))) |> 
#   filter(Station != "Pacific")





## The dates are 4 5 6 18 19 July 27 28 Jun

# douglas_dates <- blank_comments |>
#   filter(as_date(TimeOfInspection) %in% as.Date(c(
#     "2025-07-04",
#     "2025-07-05",
#     "2025-07-06",
...
## For those dates - lets set the location to Douglas!
...
p1 <- ggplot() +
  tidyterra::geom_spatraster_rgb(
    data = bc_stations_basemap_colour
  ) +
  ...
  scale_pattern_manual(
    name = "2025 Watercraft\nInspection Stations",

    values = c(
      "Permanent Inspection Station" = "none",
      "Part-time Inspection Station" = "stripe",
      "Roving Inspection Crew" = "circle"
    )
  ) +
  ...
  scale_fill_manual(
    ...
    values = c(
      "Permanent Inspection Station" = "#0072B2",
      "Part-time Inspection Station" = "#F0E442",
      "Roving Inspection Crew" = "#CC79A7"
    )
  ) +
...
# p1<-ggplot()+ tidyterra::geom_spatraster_rgb(data = bc_stations_basemap_colour)+ geom_sf(data = st_buffer(stations_for_maps,20000), aes(fill = as.factor(station_type)))+ geom_sf_label(data = station_labels, aes(label = map_label), size = 3, fill = "white", color = "black", label.size = 0.1)+ scale_fill_manual(name = "2025 Watercraft \n Inspection Stations", values = c( "Permanent Inspection Station" = "lightblue", "Roving Inspection Crew" = "purple", "Part-time Inspection Station" = ' #E8DE51'))+ ggspatial::annotation_scale()+ ggthemes::theme_map()+ theme(legend.position = c(0.68,0.72), legend.background = element_rect(colour = 'black') ) p1
...
my_colors = RColorBrewer::brewer.pal(n = 5, name = "Spectral")[1:5]


p5 = ggplot() +
  tidyterra::geom_spatraster_rgb(data = bc_stations_basemap_colour) +
  geom_sf(
    data = stations_rs_sf |> dplyr::arrange(desc(num_insp_b)),
    aes(fill = factor(num_insp_b), size = log(num_insp_b)+1),
    shape = 21, color = "black"
  ) +
...
dest_wb = read_sf(paste0('Z:/2 SCIENCE - Invasives/AIS_R_Projects/CMadsen_Wdrive/Projects/ZQMussels/',my.year,' IMDP Final Report/data/spatial/Waterbodies_with_Inspection_Data_Summaries_',my.year,'.gpkg')) |> 
...
mf_time_table = read_excel("Z:/2 SCIENCE - Invasives/SPECIES/Zebra_Quagga_Mussel/Communications/Inspection data reporting/Final report/2020/GIS Maps and Excel Figures/ExcelFigureData/2019_Fig24_MusselFouled_byMonth_2017-2020.xlsx",sheet = "dat_for_R")
...
if(my.year == 2025){
  p25_dat = tibble(Province_or_State = factor(c("Ontario", "North Dakota")),
       NumberBoats = c(5,1))
}
...
if(my.year == 2025){
    p26 = tibble(Region = factor(c("Skeena", 
                                   "Thompson \nOkanagan",
                                   "Alaska",
                                   "South Coast"
                                   )),
