...
permanent.stations = c("Golden","Radium","Olsen","Yahk",
                        "Osoyoos","Mt. Robson",
                        "Dawson Creek")

roving.stations = c("Pacific","Penticton Roving","Lower Mainland Roving",
                    "Keremeos (Hwy 3)","Sumas Border","Cutts (Hwy 93)")
...
bc_stations_basemap_white = maptiles::get_tiles(x = bc_station_view, provider = 'CartoDB.Positron', zoom = 6, crop = F)
...
stations_roving = stations_roving |> 
  filter(station_type != 'Permanent Inspection Station' | station_name == 'Cutts (Hwy 93)') |> 
...
station_labels_roving = stations_roving |> 
  ...
  dplyr::mutate(lat = case_when(
    ...
    map_label == 'Sumas Border' ~ lat - 0.4,
    map_label == 'Keremeos' ~ lat - 0.4,
    T ~ lat)) |> 
  dplyr::mutate(lon = case_when(
...
if(!interactive()) {
ggsave("Fig3b_EncounterFrequency_HR_RovingStations.jpg",p3_c)
}
```
...
if(!interactive()) {
ggsave(paste0("Fig3x_c_TotalInspections_",my.year-2,"_to_",my.year,"_Roving_Stations.jpg"),p3.x_c)
}
```
...
### Roving Station but not Boat Launch nor
### Figure 13 Noncompliant Watercraft Breakdown
```{r fig13_noncompliant_breakdown}
# Make a version of our dataset that adopts temporary changes to 
# station names based on shift start comments; if they mention 
# a boat launch / lake name, change the station to 'Boat Launch'
dat_bl = dat |> 
  mutate(Station = ifelse(str_detect(Shift_Start_Comment,'(lake|Lake|boat l|Boat L)'),'Boat Launch',Station))



dat_bl |> 
  filter(str_detect(Station, "Scheduled") | Station %in% c("Lower Mainland Roving","Penticton Roving","Boat Launch")) |>
  select(Shift_ID,
         Station,
  ...
  group_by(Station) |> 
  dplyr::summarise(
    nonmotorized_blowbys = sum(as.numeric(Non_Motorized_Blow_Bys_Counter)),
    motorized_blowbys = sum(as.numeric(Motorized_Blow_Bys_Counter)),
    number_rows = n()) |> 
  ungroup() |> 
  mutate(Station = paste0(Station, " (N = ",number_rows,")")) |> 
  knitr::kable()

p13 = dat_bl |> 
  filter(!Station %in% c(rovers_to_drop)) |> 
...
dest_wb_hr = dest_wb_hr |>
  mutate(GNIS_NA_label = ifelse(as.numeric(HighRisk_b) < 3, NA, GNIS_NA))
...
map_6_tmap = tm_shape(bc_stations_basemap_white, 
...
map_9_tmap = tm_shape(bc_stations_basemap_white, bbox = bc_station_view) +
    tm_rgb() + 
