...
# leaflet maps?
leaflet.stations.to.include = c("Golden","Radium","Olsen","Dawson Creek","Mt. Robson","Penticton Roving","Osoyoos","Fraser Valley Roving")
...
if(my.year == 2022){
  ...
  rovers_to_drop = c("Scheduled Inspection","Other","Okanagan")
}
```
...
map_1_tmap = tm_shape(bc_stations_basemap, bbox = bc_station_view) +
    tm_rgb() + 
  tm_add_legend(title = '2022 Watercraft \nInspection Stations') + 
  tm_shape(bc_bound) + 
...
map_2_tmap = tm_shape(bc_stations_basemap, bbox = bc_station_view) +
    tm_rgb() + 
  tm_add_legend(title = '2022 Inspections \nby Stations',
                type = 'symbol',
...
# insp_by_week_and_month
# 
# dat %>% 
#   # dplyr::filter(Shift_hours > 1) |> 
...
#            the.day) %>% 
#   summarise(Number_Insp = n(),
#             TotalEffort = sum(Shift_hours,na.rm=T)
#   ) |> 
#   dplyr::mutate(enc_freq = Number_Insp / TotalEffort) |> 
#   dplyr::arrange(desc(enc_freq)) 
#   dplyr::group_by(the.day) |>
#   dplyr::summarise(
#     mean(Number_Insp/TotalEffort),
#     sd = sd(Number_Insp / TotalEffort, na.rm=T),
#     StandardError = plotrix::std.error(Number_Insp / TotalEffort))
  

p8 = dat %>% 
  # Replace 0's for shift hours with the average: 8
  mutate(Shift_hours = ifelse(Shift_hours == 0, 8, Shift_hours)) |> 
  mutate(the.day = wday(TimeOfInspection,
                        label=T,abbr=F,
                        week_start = getOption("lubridate.week.start", 1)),
         the.week = week(TimeOfInspection),
...
# Add in proportion of blow-bys that are non-motorized
# 2026/(2026+428)
```
...
map_4_tmap = tm_shape(bc_stations_basemap, bbox = bc_station_view) +
    tm_rgb() + 
  tm_add_legend(title = '2022 High Risk \nInspections',
                type = 'symbol',
                labels = unique(stations_sf$bin_label),
                col = my_pal(unique(stations_sf$num_insp_b)),
                size = log(unique(stations_sf$num_insp_b)+1)) + 
...
if(interactive()){
  # Get number of high-risk inspections that had been previously inspected.
  dat_hr |> 
    filter(Year == my.year) |> 
...
map_5_tmap = tm_shape(northamerica_basemap, 
                      bbox = northamerica_view) +
    tm_rgb() + 
  tm_add_legend(title = '2022 Source of \nHigh Risk Inspections',
                type = 'symbol',
                labels = unique(centr_with_dat$bin_label),
                col = unique(centr_with_dat$my_colors),
...
p25 = tibble(Province_or_State = factor(c("Ontario","Manitoba","Quebec")),
...
map_8_tmap = tm_shape(northamerica_basemap, 
                      bbox = northamerica_view) +
    tm_rgb() + 
  tm_add_legend(title = '2022 Commercially \nHauled Inspections',
                type = 'symbol',
                labels = unique(centr_with_dat$bin_label),
                col = unique(centr_with_dat$my_colors),
...
map_9_tmap = tm_shape(bc_stations_basemap, bbox = bc_station_view) +
    tm_rgb() + 
  tm_add_legend(title = '2022 Invasive \nMussel Lake Monitoring',
                type = 'symbol',
...
```{r number_nonmotorized_non_compliers, include=F}
...
```{r previous_CDD_AIS_knowledge_numbers, include=F}

# What percentage of inspections had previous knowledge of AIS / CDD?

( nrow(dat %>% filter(Previous_AIS_Knowledge_Ind == T)) ) /
  ( nrow(dat) )

# And which sources of knowledge were most salient?
dat %>% 
...
```{r calculation_of_boat_types, include=F}
