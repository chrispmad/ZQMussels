...
part.time.stations = c("Sumas Border","Pacific","Cutts (Hwy 93)","Radium")
...
station_labels = stations_for_maps |> 
  ...
  dplyr::mutate(lat = case_when(
    ...
    map_label == "Peace Arch" ~ lat - 0.2,
    T ~ lat)) |> 
  dplyr::mutate(lon = case_when(
    map_label == 'Lower Mainland' ~ lon - 1,
    map_label == 'Penticton' ~ lon + 1.5,
    map_label == "Peace Arch" ~ lon - 1,
    map_label == "Sumas Border" ~ lon + 1,
    T ~ lon)) |> 
...
ggplot() +
  ...
  geom_sf(
    data = centr_with_dat,
    aes(fill = factor(TotalInsp_b), size = TotalInsp_b),
    shape = 21,
    color = "black"
  ) +
  geom_sf_text(
  ...
  scale_fill_manual(
    name = paste0(my.year, paste0(my_opts$year,' Home Residence \nFrequency')),
